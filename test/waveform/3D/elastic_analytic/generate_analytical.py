...
ALPHA = 3500.0     # P-wave velocity (m/s)
BETA = 2000.0      # S-wave velocity (m/s)
...
# Original distances from scaling_results benchmark (r = 5000-7280 m)
STATIONS = [
    # name,  x_AR(North), y_AR(East), z_AR(Down)
    ("R1",   1000.0,      3000.0,     4000.0),
    ("R2",   3000.0,      1000.0,     4000.0),
    ("R3",   1000.0,      6000.0,     4000.0),
    ("R4",   6000.0,      1000.0,     4000.0),
    ("R5",   1000.0,      6000.0,     2000.0),
    ("R6",   6000.0,      1000.0,     2000.0),
]
...
def compute_analytical(x_north, y_east, z_down):
    ...
    Returns:
        ...
        u_east, u_north, u_up: displacement in SEMSWS convention
    """
    ...
    # Convert to SEMSWS convention: x=East, y=North, z=Up
    u_x = u_east
    u_y = u_north
    u_z = -u_down  # Up = -Down
...
def main():
    ...
    for name, x_n, y_e, z_d in STATIONS:
        ...
        sem_z = SRC_Z - z_d   # Up
        print(f"  SEMSWS position: ({sem_x}, {sem_y}, {sem_z})")

        times, u_x, u_y, u_z = compute_analytical(x_n, y_e, z_d)

        # Write reference files in SEMSWS format: time amplitude
        for comp, data in [("x", u_x), ("y", u_y), ("z", u_z)]:
            fname = os.path.join(ref_dir, f"{name}_{comp}.d")
            with open(fname, 'w') as f:
                for t, u in zip(times, data):
                    f.write(f"  {t:14.8E}   {u:14.8E}\n")

        print(f"  Written: {name}_[x,y,z].d")
        print()
    ...
    print("SEMSWS receiver locations for config.yaml:")
    for name, x_n, y_e, z_d in STATIONS:
        sem_x = SRC_X + y_e
        sem_y = SRC_Y + x_n
        sem_z = SRC_Z - z_d
