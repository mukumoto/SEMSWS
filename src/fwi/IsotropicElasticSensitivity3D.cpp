/**
 * @file IsotropicElasticSensitivity3D.cpp
 * @brief Hand-written sensitivity kernel for 3D isotropic elastic media.
 *
 * Same structure as IsotropicElasticSensitivity2D with forall_3D, vector
 * (ux, uy, uz) gathers, divergence = 3-term sum, strain contraction over
 * the full symmetric 3×3 tensor (6 independent components), and the scalar
 * ρ-path built from ü·λ*.
 */
...
IsotropicElasticSensitivity3D::IsotropicElasticSensitivity3D(
...
void IsotropicElasticSensitivity3D::InitCoefficients(
    const MaterialField3D& lambda,
    const MaterialField3D& mu,
    const MaterialField3D& rho,
    const MaterialField3D* c_kappa,
    const MaterialField3D* c_mu)
{
    ...
    mfem::forall(total_gll, [=] MFEM_HOST_DEVICE (int i) {
        ...
        // 3D: κ = λ + 2μ/3. κ_u = c_κ κ_user, μ_u = c_μ μ_user.
        ...
        d_rho_out[i]       = r;
        // Pure elastic form — see IsotropicElasticSensitivity2D.cpp for why
        // c_κ / c_μ must NOT multiply the Save-time coefficients for SLS
        // visco-elastic (SLS memory makes the accumulated K_λ/K_μ probe
        // λ_user/μ_user, not the stored unrelaxed moduli). In 3D with
        // κ=λ+2μ/3, pure-elastic chain rule reduces to:
        //   K_Vp = 2 ρ Vp · K_λ,   K_Vs = 2 ρ Vs · (K_μ − 2·K_λ)
        // (coeff_vs_lam = -4 ρ Vs matches the 2D formula as well.)
        (void)ck; (void)cm;
...
void IsotropicElasticSensitivity3D::Reset() {
    k_lambda_ = 0.0; k_mu_ = 0.0; k_rho_ = 0.0;
}

void IsotropicElasticSensitivity3D::ResetHessian() {
...
void IsotropicElasticSensitivity3D::Accumulate(
...
//   1. Load (ux, uy, uz) and (λ*x, λ*y, λ*z) into shared memory.
//   2. Compute 9 reference gradients → 9 physical gradients via packed invJ.
//   3. Divergences: ∇·u = ∂ux/∂x + ∂uy/∂y + ∂uz/∂z   (and same for λ*).
//      Strain contractions:
//          ε:ε' = Σ_i ε_ii·ε'_ii + 2 Σ_{i<j} ε_ij·ε'_ij
//      where ε_ij = (∂u_i/∂x_j + ∂u_j/∂x_i) / 2.
//   4. K_λ -= (∇·u)(∇·λ*) dt ; K_μ -= 2 ε:ε' dt ; K_ρ -= (a·λ*) dt.
// =============================================================================

template <int NGLL>
void IsotropicElasticSensitivity3D::AccumulateKernel(
    const Vector& fwd_u, const Vector& fwd_a,
    const Vector& adj_u, real_t dt)
{
    const int ne = ne_;
        ...
        MFEM_FOREACH_THREAD(iz, z, NGLL) {
            MFEM_FOREACH_THREAD(iy, y, NGLL) {
                MFEM_FOREACH_THREAD(ix, x, NGLL) {
                    ...
                    // Packed invJ (0-8 row-major, 9 = detJ).
                    ...
                    // Divergences.
                    const real_t div_u = dux_dx + duy_dy + duz_dz;
                    const real_t div_l = dlx_dx + dly_dy + dlz_dz;

                    // Off-diagonal symmetric strain entries.
                    const real_t exy_u = 0.5 * (dux_dy + duy_dx);
                    const real_t exz_u = 0.5 * (dux_dz + duz_dx);
                    const real_t eyz_u = 0.5 * (duy_dz + duz_dy);
                    const real_t exy_l = 0.5 * (dlx_dy + dly_dx);
                    const real_t exz_l = 0.5 * (dlx_dz + dlz_dx);
                    const real_t eyz_l = 0.5 * (dly_dz + dlz_dy);

                    // ε(u):ε(λ*) full contraction.
                    const real_t eps_eps =
                        dux_dx * dlx_dx + duy_dy * dly_dy + duz_dz * dlz_dz
                        + 2.0 * (exy_u * exy_l + exz_u * exz_l + eyz_u * eyz_l);

                    // Forward self-contraction for pseudo-Hessian.
                    const real_t eps_sq_u =
                        dux_dx * dux_dx + duy_dy * duy_dy + duz_dz * duz_dz
                        + 2.0 * (exy_u * exy_u + exz_u * exz_u + eyz_u * eyz_u);
...
void IsotropicElasticSensitivity3D::SaveHessian(
    const std::string& dir, ParMesh& /*mesh*/, int source_id)
{
    const int total = ngll_ * ngll_ * ngll_ * ne_;
...
#define INSTANTIATE_ELASTIC_SENS_3D(NGLL) \
    template void IsotropicElasticSensitivity3D::AccumulateKernel<NGLL>( \
        const Vector&, const Vector&, const Vector&, real_t);

SEM_INSTANTIATE_NGLL_3D(INSTANTIATE_ELASTIC_SENS_3D)

#undef INSTANTIATE_ELASTIC_SENS_3D
