/**
 * @file WaveOperatorBase2D.cpp
 * @brief Implementation of 2D wave operator base class
 */
...
WaveOperatorBase2D::WaveOperatorBase2D(ParFiniteElementSpace& fes, int order)
...
WaveOperatorBase2D::~WaveOperatorBase2D() {
...
WaveOperator& WaveOperatorBase2D::SetupSource(PointSourceCollection& source) {
    source_ = &source;
    return *this;
}

void WaveOperatorBase2D::ResetState() {
    current_step_ = 0;
    ResetPhysicsState();
}

void WaveOperatorBase2D::AssemblePreCouplingRHS(
...
void WaveOperatorBase2D::FinalizeAndApplyMass(
...
void WaveOperatorBase2D::ExplicitSolve(
...
void WaveOperatorBase2D::AssembleDiagonalMass(const Vector& coef, bool is_vector) {
    ...
    mass_integ_ = std::make_unique<SEMMassIntegrator2D>();
    {
...
void WaveOperatorBase2D::AssembleDomainRHS(const ParGridFunction& u) {
...
void WaveOperatorBase2D::ApplyInverseMass(
    const ParGridFunction& dudt,
    ParGridFunction& dudt2)
{
    // ü = M⁻¹·(F - K·u)
...
void WaveOperatorBase2D::SetupCerjanTaper(const DampingConfig& config) {
    ...
    Vector gll_taper;
    damping::ComputeCerjanTaper2D(fes_, config, ngll, gll_taper);

    // Print statistics
    damping::PrintCerjanStats(gll_taper, fes_.GetComm());

    // Project taper into DOF space using FunctionCoefficient
    // We need physical coordinates, so build a per-DOF taper from the GLL data.
    // For nodal GLL basis, element DOFs correspond to GLL points directly.
    ...
    *cerjan_taper_ = 1.0;
    ...
    for (int ie = 0; ie < ne; ie++) {
        ...
        const int ndof_per_comp = ngll * ngll;
        ...
        for (int j = 0; j < ndof_per_comp; j++) {
            ...
            for (int c = 0; c < vdim; c++) {
                ...
                // Take min (strongest damping) for shared DOFs
...
void WaveOperatorBase2D::ApplyCerjanTaper(
...
void WaveOperatorBase2D::FreeSetupResources() {
...
void WaveOperatorBase2D::DeviceInit() {
    // Initialize device memory for all domain integrators
    for (int i = 0; i < domain_integs_.Size(); i++) {
        auto* sem_integ = static_cast<SEMIntegratorBase2D*>(domain_integs_[i]);
