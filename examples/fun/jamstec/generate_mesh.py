"""
Generate HOHQMesh control file for "jamstec" text mesh.

Domain: 12000m x 3000m (double width for 7 letters vs 3)
Letters "jamstec" (without dot on j) as inner boundaries (holes).
Source will be placed at the "j dot" position.
...
def ensure_ccw(contour):
    """Ensure contour is counter-clockwise (required by HOHQMesh)."""
...
def find_refinement_centers(contours, element_size, angle_threshold=25.0):
    """Find high-curvature points on contours that need refinement."""
    all_sharp = []
    for c in contours:
        pts = c[:-1]
        d1 = np.diff(pts, axis=0)
        d1 = np.vstack([d1, pts[0] - pts[-1]])
        d2 = np.roll(d1, -1, axis=0)
        cross = d1[:, 0] * d2[:, 1] - d1[:, 1] * d2[:, 0]
        dot = d1[:, 0] * d2[:, 0] + d1[:, 1] * d2[:, 1]
        angles = np.abs(np.arctan2(cross, dot)) * 180 / np.pi
        for idx in np.where(angles > angle_threshold)[0]:
            all_sharp.append((pts[idx, 0], pts[idx, 1], angles[idx]))

    # Cluster nearby points (within 1.5 * element_size)
    all_sharp.sort(key=lambda x: x[0])
    clusters = []
    for x, y, ang in all_sharp:
        merged = False
        for cl in clusters:
            if abs(cl[0] - x) < element_size * 1.5 and abs(cl[1] - y) < element_size * 1.5:
                if ang > cl[2]:
                    cl[0], cl[1], cl[2] = x, y, ang
                merged = True
                break
        if not merged:
            clusters.append([x, y, ang])

    return clusters


def write_control_file(contours, domain_w, domain_h, element_size,
                       output_dir, poly_order=4, refinement_centers=None):
    """Write HOHQMesh control file with jamstec inner boundaries."""
    ctrl_path = f"{output_dir}/jamstec.control"
    mesh_name = f"{output_dir}/jamstec"

    with open(ctrl_path, "w") as f:
        f.write("%\n")
        f.write("% HOHQMesh control file for 'jamstec' text mesh\n")
        ...
        # Refinement regions around sharp corners
        if refinement_centers:
            f.write("   \\begin{REFINEMENT_REGIONS}\n\n")
            for i, (x, y, ang) in enumerate(refinement_centers):
                h = element_size * 0.5  # target size: 50% of background
                w = element_size * 3.0  # influence width (smooth transition)
                f.write(f"      \\begin{{REFINEMENT_CENTER}}\n")
                f.write(f"         type = smooth\n")
                f.write(f"         x0   = [{x:.1f}, {y:.1f}, 0.0]\n")
                f.write(f"         h    = {h:.1f}\n")
                f.write(f"         w    = {w:.1f}\n")
                f.write(f"      \\end{{REFINEMENT_CENTER}}\n\n")
            f.write("   \\end{REFINEMENT_REGIONS}\n\n")

        f.write("   \\begin{SPRING_SMOOTHER}\n")
        f.write("      smoothing            = ON\n")
        f.write("      smoothing type       = LinearAndCrossBarSpring\n")
        f.write("      number of iterations = 60\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate HOHQMesh control file for 'jamstec' text mesh")
    parser.add_argument("--domain-w", type=float, default=20000.0,
                        help="Domain width [m]")
    parser.add_argument("--domain-h", type=float, default=5000.0,
                        help="Domain height [m]")
    parser.add_argument("--letter-height", type=float, default=2750.0,
                        help="Letter height [m]")
    parser.add_argument("--element-size", type=float, default=100.0,
    ...
    print("Extracting letter contours for 'jamstec'...")
    contours_raw = get_letter_contours("jamstec", font_size=72,
    ...
    contours_scaled = filter_and_scale_contours(
        ...
        args.letter_height, min_area_ratio=0.003)

    # Remove contours that are INSIDE another contour (holes in a, e, etc.)
    # HOHQMesh crashes with nested inner boundaries.
    # Detection: if the centroid of a smaller contour is inside a larger one.
    from matplotlib.path import Path as MplPath
    to_remove = set()
    for i in range(len(contours_scaled)):
        if i in to_remove:
            continue
        ai = _contour_area(contours_scaled[i])
        for j in range(len(contours_scaled)):
            if i == j or j in to_remove:
                continue
            aj = _contour_area(contours_scaled[j])
            if ai >= aj:
                continue
            # Check if centroid of i is inside contour j
            ci = contours_scaled[i][:-1].mean(axis=0)
            path_j = MplPath(contours_scaled[j][:-1])
            if path_j.contains_point(ci):
                to_remove.add(i)
                print(f"  Removing nested hole contour {i} "
                      f"(inside contour {j}, area={ai:.0f} m²)")
                break
    if to_remove:
        contours_scaled = [c for idx, c in enumerate(contours_scaled)
                           if idx not in to_remove]
        print(f"  After removing nested holes: {len(contours_scaled)} contours")
    print(f"  Scaled contours (filtered): {len(contours_scaled)}")

    # Remove the dot of 'j' — find the small contour with leftmost + highest y
    # 'j' is the first letter, so its dot should be the leftmost small high contour
    centroids = [(c[:-1].mean(axis=0), i) for i, c in enumerate(contours_scaled)]
    areas = []
    for c in contours_scaled:
        x, y = c[:-1, 0], c[:-1, 1]
        areas.append(abs(0.5 * np.sum(x * np.roll(y, -1) - np.roll(x, -1) * y)))

    # The j-dot is a small contour near the top-left
    max_cy = max(c[0][1] for c in centroids)
    min_cx = min(c[0][0] for c in centroids)
    dot_candidates = []
    for (cxy, idx), area in zip(centroids, areas):
        # Small area, high y, leftmost region
        if cxy[1] > max_cy - 300 and cxy[0] < min_cx + 1500:
            dot_candidates.append((idx, area, cxy))

    if dot_candidates:
        # Pick smallest area among candidates
        dot_idx, dot_area, dot_cxy = min(dot_candidates, key=lambda x: x[1])
        dot_center = contours_scaled[dot_idx][:-1].mean(axis=0)
        print(f"  Removed 'j' dot (contour {dot_idx}), "
              f"center: ({dot_center[0]:.0f}, {dot_center[1]:.0f}), "
              f"area: {dot_area:.0f} m²")
        ...
        np.savetxt(f"{args.output_dir}/source_position.txt",
                   ...
                   header="x y (position of j-dot, use as source location)")
    ...
    # Find refinement centers at sharp corners
    ref_centers = []  # Disabled: refinement worsens mesh quality for this case
    print(f"  Refinement centers: {len(ref_centers)} (disabled)")

    # Write control file
    write_control_file(contours_scaled, args.domain_w, args.domain_h,
                       args.element_size, args.output_dir, args.poly_order,
                       refinement_centers=ref_centers)
