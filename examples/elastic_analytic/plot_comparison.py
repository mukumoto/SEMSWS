#!/usr/bin/env python3
"""Plot comparison of analytical vs synthetic seismograms for paper."""
...
# Z1 (5 km), Z5 (6.4 km), Z4 (7.2 km)
stations = ["Z1", "Z5", "Z4"]
offsets = ["5.0 km", "6.4 km", "7.2 km"]
comps = ["x", "y", "z"]
comp_labels = ["X", "Y", "Z"]
comp_to_ref = {"x": "FXX", "y": "FXY", "z": "FXZ"}
...
for i, (sta, offset) in enumerate(zip(stations, offsets)):
    for j, comp in enumerate(comps):
        ...
        # Load ref (analytic)
        ref_data = np.loadtxt(os.path.join(ref_dir, f"XX.{sta}.{comp_to_ref[comp]}.semd"))
        ...
        # Normalize by scale factor from analytic solution
        ...
        ax.plot(t, u_ref / scale, 'k', lw=1.5, label='Analytic')
        ax.plot(t, u_syn / scale, 'r', lw=0.8, linestyle='--', label='Numerical')
        ...
        if j == 0:
            ax.set_ylabel(f'Offset {offset}', fontsize=11)
        if i == 0 and j == 2:
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, "comparison_paper.png"), dpi=150, bbox_inches='tight')
plt.savefig(os.path.join(script_dir, "comparison_paper.pdf"), bbox_inches='tight')
plt.close()
print("Saved: comparison_paper.png, comparison_paper.pdf")
