#!/bin/bash
# ============================================================
# 3D Elastic 2-Layer Benchmark — visco-elastic variant
#
# Usage:
#   bash run_visco.sh [NPROCS]         # default NPROCS=4
#
# Prereqs:
...
echo "=========================================="
echo "3D Visco-Elastic 2-Layer Benchmark (CPU)"
echo "NP=${NP}"
...
mpirun -np "${NP}" "${progdir}/refine_mesh" --config config_visco.yaml --output-dir mesh

# Step 2: Evaluate refined mesh (optional diagnostics)
echo ""
echo "Step 2: Evaluate mesh"
mpirun -np "${NP}" "${progdir}/evaluate_mesh" --config config_visco_sim.yaml --histogram eval

# Step 3: Forward simulation
echo ""
echo "Step 3: Run simulation"
mpirun -np "${NP}" "${progdir}/test_forward_simulation" --config config_visco_sim.yaml
